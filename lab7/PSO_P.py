...
minPoblacion = -10 # valor minimo de la poblacion
maxPoblacion = 10 # valor maximo de la poblacion
tamPoblacion = 10 # tamaño de la poblacion
c1 = 0.35 
c2 = 1.2
dimProblema = 2
w = 0.21
maxIteracion = 100 # num de iteraciones
...
resultados = []
logro = []
fitnessPuntos = [[100, 100] for _ in range(tamPoblacion)] #[x][0] --> valor fitness de la iteracion [x][1] valor fitness de la mejor position

# ------------------------------------------------- funciones -----------------------------------------
def fitness(x,y):
    #return -math.cos(x) * math.cos(y) * math.exp(-(x - math.pi)**2 - (y - math.pi)**2) #eason
    #return -(sum(i * math.cos((i + 1) * x + 1) for i in range(1, 6))) * (sum(i * math.cos((i + 1) * y + 1) for i in range(1, 6))) # shubert
...
for a in range(0,50):
    ...
    for z in range(1, maxIteracion + 1):
        ...
        for i in range(0, tamPoblacion):
            ...
            if( round(fitnessPuntos[mejorPosition][1]) == 0 or z == maxIteracion): # <-----------------
                logro.append(z)
        print("\n")
    resultados.append(fitnessPuntos[mejorPosition][1])
cont = 0
for i in range(0,50):
    print(str(resultados[i])) # comentar esta linea <---------------------------------------
    if( round(resultados[i]) == 0): # cambiar< -------------------------------
        cont = cont + 1

print("exito: " + str(cont * 2) + "\% \npromedio:" + str(statistics.mean(logro)) + "\n ---- desviacion: " + str(statistics.stdev(logro)))
